...
def download_image(url, filename):
    if os.path.exists(filename):
        return
    create_directory(filename.split("/")[0])
...
def main():
    ...
    base_directory = "characters/" if ran_normally else "assets/genshin/characters/"
    ...
    for character_id in base_data.data.items:
        ...
        use_directory = f"{base_directory}{str(Elements(char.element)).lower()}/{char.name.lower().replace(' ', '_')}"

        create_directory(use_directory)

        if char.name == "Traveler":
            download_image(f"{base_asset_directory}{char.icon}.png",
                           f"{use_directory}/{'icon_boy' if 'PlayerBoy' in char.icon else 'icon_girl'}.png")
            continue
        ...
        # Namecard Icon
        download_image(f"{base_asset_directory}namecard/UI_NameCardIcon_{namecard_name}.png",
                       f"{use_directory}/namecard_icon.png")
        download_image(f"{base_asset_directory}namecard/UI_NameCardPic_{namecard_name}_P.png",
                       f"{use_directory}/namecard_picture.png")
        ## Special Food
        if char.other.special_food is not None:
            download_image(f"{base_asset_directory}/{char.other.special_food.icon}.png",
                           f"{use_directory}/special_food.png")
        ## Gacha Art
        download_image(f"{base_asset_directory}UI_Gacha_AvatarImg_{namecard_name}.png",
                       f"{use_directory}/gacha_art_full.png")

        ## Constellation Icons
        for i in char.constellation:
            download_image(f"{base_asset_directory}{char.constellation[i].icon}.png",
                           f"{use_directory}/constellation_{i}.png")
        ## Talent Icons
        for i in char.talent:
            download_image(f"{base_asset_directory}{char.talent[i].icon}.png", f"{use_directory}/talent_{i}.png")

        ## Honeyhunter Stuff
        aaa = char.icon.split("_")[-1].split(".")[0]
        # Gacha Card
        download_image(f"{base_honey_directory}{aaa.lower()}_{str(char.id)[-3:]}_gacha_splash.webp",
                       f"{use_directory}/gacha_art_cropped.png")
        download_image(f"{base_honey_directory}{aaa.lower()}_{str(char.id)[-3:]}_gacha_card.webp",
                       f"{use_directory}/gacha_art_card.png")
        # Side Icon
        download_image(f"{base_honey_directory}{aaa.lower()}_{str(char.id)[-3:]}_side_icon.webp",
                       f"{use_directory}/icon_side.png")
...
if __name__ == "__main__":
    ...
    from assets.genshin.ambr_classes.avatar_base import avatar_base_from_dict
    from assets.genshin.ambr_classes.avatar_data import avatar_data_from_dict
