from ambr_wrapper_test.artifacts_base import artifacts_base_from_dict
from ambr_wrapper_test.artifacts_data import artifacts_data_from_dict
...
for artifact_set in tqdm(base_data.data.items):
    ...
    print(f" Downloading: {full_set.name}")
    create_directory(f"{base_directory}{str(full_set.name).lower().replace(' ', '_')}/")
    for i in [full_set.suit.equip_bracer, full_set.suit.equip_dress, full_set.suit.equip_necklace, full_set.suit.equip_ring, full_set.suit.equip_shoes]:
        try:
            download_image(f"{base_asset_directory}reliquary/{i.icon}.png", f"{base_directory}{str(full_set.name).lower().replace(' ', '_')}/{parse_artifacts(i.icon)}.png")
        except:
            pass
